...
# FlameLogger.raise_error only logs and then sleeps (fail-slow, so the platform
# can reap the container). Surface the error here so the `pytest.raises` case
# asserts the lookup failure rather than blocking.
...
def test_data_api_client_init():
    with patch(
        "flamesdk.resources.client_apis.clients.data_api_client.AsyncClient.get",
        new=AsyncMock(side_effect=dummy_get),
    ):
        flame_logger = FlameLogger()
        client = DataApiClient(
            "proj_id", "nginx", "data_token", "key_token", flame_logger
        )
...
def test_get_data_fhir():
    ...
    with patch(
        ...
        expected = {
            "query1": {"result": "fhir-data"},
            "query2": {"result": "fhir-data"},
        }
...
def dummy_sources(monkeypatch):
    ...
    async def dummy_retrieve_available_sources(
        self, default_requires_data: bool = True
    ):
        return sources

    monkeypatch.setattr(
        DataApiClient, "_retrieve_available_sources", dummy_retrieve_available_sources
    )
    return sources
